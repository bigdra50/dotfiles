...
期待値をファイルに保存し、出力と比較するパターン。シリアライズやテキスト出力の検証に有用。

```csharp
[Test]
public void Serialize_ValidData_MatchesGoldenFile()
{
    var sut = CreateTestData();

    var actual = sut.Serialize();

    var expected = File.ReadAllText("Assets/Tests/TestData/golden/expected_output.json");
    Assert.That(actual, Is.EqualTo(expected));
}
```

- 期待値ファイルは `Tests/TestData/` ディレクトリに配置
- ファイル名は `golden/` サブディレクトリで管理
...
```bash
# Edit Mode tests
uvx --from git+https://github.com/bigdra50/unity-mcp-client unity-mcp tests EditMode

# Play Mode tests
uvx --from git+https://github.com/bigdra50/unity-mcp-client unity-mcp tests PlayMode

# フィルタ指定（クラス名やメソッド名でフィルタ）
uvx --from git+https://github.com/bigdra50/unity-mcp-client unity-mcp tests EditMode --filter "CharacterControllerTest"
```
