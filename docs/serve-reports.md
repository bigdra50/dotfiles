...
mise run serve-reports -- github.com/<owner>        # 任意のホスト/owner 指定
...
- 集約先はタスク実行のたびに作り直される (手動でファイルを置かないこと)
...
2. 管理されたネットワークのセキュリティスキャナによるポートプローブ
...
クライアント分離 (AP isolation) が有効なネットワークが原因の可能性が高い。
