...
実装は `scripts/reference/`。
パース・diff は `scripts/reference/keybindings/`、レンダリングは共通の `scripts/reference/common/`（列駆動の汎用レンダラ）。いずれも純粋関数（Python 3.14 標準ライブラリのみ）で、プロセス起動とファイルI/OはCLIラッパとbashに分離している。
